...
class LogPolytopeBarrier:
    ...
    def _get_dists(self, xs):
        ...
        xs_len_along_ws = xs.dot(self.ws.T)/np.linalg.norm(self.ws, axis=1)
        hyperplane_dist = self.bs/np.linalg.norm(self.ws, axis=1)
        dists = xs_len_along_ws - hyperplane_dist # dists.shape = (N_x, N_ws)
        signs = 2*(dists * np.sign(hyperplane_dist) > 0) - 1
        return np.abs(dists), signs
    ...
    def f1(self, xs):
        dists, signs = self._get_dists(xs)
        grads = (1/dists * signs).dot((-self.ws / np.linalg.norm(self.ws, axis=1).T.reshape(-1, 1)))
        return grads

    def f2(self, xs):
        a = time.time()
        normalized_ws = self.ws / np.linalg.norm(self.ws, axis=1).reshape(-1, 1)
        dists, signs = self._get_dists(xs)
        hess = []
        for i in range(len(xs)):
            hess.append(np.dot(normalized_ws.T, 1/(dists[i].reshape(-1, 1))**2 * normalized_ws))
